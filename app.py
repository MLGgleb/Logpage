    ...
    if request.method == 'POST':
        if request.form.get('login'):
            ...
            if password == '':
                password_exist = True
            try:
                cursor.execute("SELECT * FROM public.'service.users' WHERE login=%s AND password=%s",
...
def registration():
    ...
    if request.method == 'POST':
        ...
        if name_exist or login_exist or password_exist:
            ...
            cursor.execute('SELECT * FROM public."service.users" WHERE login = %s;', [str(user_login)])
            records = list(cursor.fetchall())
            if records:
                return render_template('registration.html', name_exist=name_exist, login_exist=login_exist,
                                       password_exist=password_exist, coincidence=True)
            else:
                cursor.execute('INSERT INTO public."service.users" (full_name, login, password) VALUES (%s, %s, %s);',
